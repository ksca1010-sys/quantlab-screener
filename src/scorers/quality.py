...
def score_quality(
    ...
    s1 = _roe_score(df)               # 30점
    s2 = _op_margin_score(df)         # 25점
    s3 = _debt_ratio_score(df)        # 25점
    s4 = _interest_coverage_score(df) # 20점
...
def _roe_score(df: pd.DataFrame) -> pd.Series:
    ...
    return (minmax_scale(roe.fillna(0)) * 30).rename(None)
...
def _op_margin_score(df: pd.DataFrame) -> pd.Series:
    ...
    return (minmax_scale(margin.fillna(0)) * 25).rename(None)
...
def _debt_ratio_score(df: pd.DataFrame) -> pd.Series:
    ...
    # 역수 변환: 부채비율 0에 가까울수록 높은 점수
    inv = 1 / (1 + ratio.fillna(500))
    return (minmax_scale(inv) * 25).rename(None)
...
def _interest_coverage_score(df: pd.DataFrame) -> pd.Series:
    ...
    return (minmax_scale(ic.fillna(0)) * 20).rename(None)
