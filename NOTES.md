...
- **배당수익률**: Naver Finance 스크래핑. 전체 분위수(섹터 무관). 허용 가능.
...
- **부채비율**: `1/(1+ratio)` 역수 변환 후 전체 minmax. 부채비율은 업종 특성이 강해 섹터 분위수가 더 이상적이나, 부채 없는 회사가 구조적으로 우수함은 보편적 사실이므로 현행 유지 가능.
- **이자보상배율**: 영업이익/금융비용. 무차입(금융비용=0) 시 100으로 cap. 타당.
...
- **주의**: 리스크가 낮은 종목(저변동, 저베타)에 높은 점수 → 방어주 편향. 동일가중(20%)으로 희석됨.
...
- [ ] 부채비율·이자보상배율도 섹터 분위수로 전환 (금융업 레버리지 특성 반영)
